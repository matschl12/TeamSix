...
class HomeControllerTest {
    ...
    public void testSearchButton() {
        ...
        Movie movie1 = new Movie("Title 1", "Description 1", Genre.ACTION);
        Movie movie2 = new Movie("Title 2", "Description 2", Genre.COMEDY);
        Movie movie3 = new Movie("Title 3", "Description 3", Genre.DRAMA);
        List<Movie> allMovies = Arrays.asList(movie1, movie2, movie3);

        // create the controller and set the data
        MyController controller = new MyController();
        controller.setAllMovies(allMovies);

        // simulate user input
        controller.searchField.setText("title 2");
        controller.searchBtn.fire();

        // check that the search results are correct
        List<Movie> expected = Arrays.asList(movie2);
        List<Movie> actual = controller.observableMovies;
